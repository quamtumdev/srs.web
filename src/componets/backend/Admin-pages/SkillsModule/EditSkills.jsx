...
const EditSkills = ({ skills, setSkillsData, closeEditModal }) => {
  ...
  useEffect(() => {
    if (skills) {
      setTitle(skills.title);
      setUrl(skills.url);
      setContent(skills.content);
      setIsActive(skills.active);
    }
  }, [skills]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `https://srs-api-six.vercel.app/api/auth/skills/skills/${skills._id}`,
        {
          method: "PUT",
          headers: {
      ...
      setSkillsData(prevData =>
        prevData.map(s => (s._id === skills._id ? updatedData : s))
      );

      closeEditModal(); // Close the modal after saving
    } catch (error) {
      setError("Error updating skills: " + error.message); // Handle error
    } finally {
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete) {
      try {
        ...
        setSkillsData(prevData => prevData.filter(s => s._id !== skills._id));
      ...
      <Modal.Body>
        ...
        <Form.Group className={`mb-3 ${styles.formControl}`}>
          ...
          <Form.Control
            ...
            placeholder="Enter Add Skills"
          />
        </Form.Group>

        {/* Stream URL */}
        <Form.Group className={`mb-3 ${styles.formControl}`}>
          <Form.Label>Skills URL</Form.Label>
          <Form.Control
            type="text"
            value={url}
        ...
        <Form.Group className={`mb-3 ${styles.formControl}`}>
          <Form.Label>Skills Descriptions</Form.Label>
          <Form.Control
            as="textarea"
            rows={4}
            value={content}
            onChange={e => setContent(e.target.value)}
            placeholder="Enter Skills Descriptions"
          />
        </Form.Group>
