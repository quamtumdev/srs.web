...
const EditExam = ({ exam, updateExamData, closeEditModal, deleteExamData }) => {
  const [show, setShow] = useState(true); // Open modal by default
  ...
  useEffect(() => {
    if (exam) {
      ...
      setSelectedStream(exam.stream);
      setSelectedGuideline(exam.guideline);
      setSelectedMarkingScheme(exam.markingScheme);
      setDescription(exam.description);
      setUniqueURL(exam.uniqueURL);
      setIsActive(exam.active === "true"); // Assuming active is stored as "true"/"false" string
    }
  }, [exam]);
  ...
  const handleClose = () => {
    setShow(false);
    closeEditModal(); // Close the modal in the parent
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        updateExamData(updatedExamData); // Update exam list in parent
  ...
  const handleDelete = async () => {
    if (window.confirm("Are you sure you want to delete this exam?")) {
      ...
      try {
        ...
        if (response.ok) {
          // Remove the deleted exam from the frontend state
          deleteExamData(exam._id); // Update parent component state to remove the exam
          handleClose(); // Close the modal
        } else {
