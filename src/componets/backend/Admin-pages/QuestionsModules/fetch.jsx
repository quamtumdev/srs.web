...
const Questions = () => {
  ...
  // Fetch questions when the selected stream changes
  // useEffect(() => {
  //   const fetchQuestions = async () => {
  //     setLoading(true); // Show loading indicator
  //     try {
  //       const url = selectedStream
  //         ? `https://srs-api-six.vercel.app/api/auth/ques/filter?stream=${selectedStream.value}`
  //         : "https://srs-api-six.vercel.app/api/auth/ques/"; // Default API call if no stream is selected

  //       const response = await axios.get(url); // Fetch questions from backend
  //       console.log("Fetched Data:", response.data);

  //       if (response.data && Array.isArray(response.data)) {
  //         setQuestionsData(response.data); // Update questions data from the response
  //       } else {
  //         setQuestionsData([]); // If no data is returned, set an empty array
  //       }
  //     } catch (error) {
  //       setError("Error fetching questions: " + error.message); // Handle errors
  //     } finally {
  //       setLoading(false); // Hide loading indicator
  //     }
  //   };

  //   fetchQuestions(); // Fetch questions on component mount and when stream changes
  // }, [selectedStream]); // Dependency array to trigger re-fetch on stream change
  useEffect(() => {
    const fetchQuestions = async () => {
      setLoading(true);
      setError(null);
