...
const EditTag = ({ tags, setTagsData, closeEditModal }) => {
  ...
  useEffect(() => {
    if (tags) {
      setTitle(tags.title);
      setUrl(tags.url);
      setContent(tags.content); // Plain text content
      setIsActive(tags.active);
    }
  }, [tags]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setTagsData(prevData =>
        prevData.map(s => (s._id === tags._id ? updatedData : s))
      );

      closeEditModal(); // Close the modal after saving
    } catch (error) {
      setError("Error updating Tags: " + error.message); // Handle error
    } finally {
  ...
  const handleDelete = async () => {
    ...
    if (confirmDelete) {
      try {
        ...
        setTagsData(prevData => prevData.filter(s => s._id !== tags._id));
